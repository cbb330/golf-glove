 ...
 ******************************************************************/
wiced_transport_cfg_t transport_cfg = { WICED_TRANSPORT_UART, /**< Wiced transport type. */
 ...
 ******************************************************************/
uint8_t golf_glove_generic_access_device_name[] = { 'g', 'o', 'l', 'f', '_',
        'g', 'l', 'o', 'v', 'e' };
uint8_t golf_glove_generic_access_appearance[] = { BIT16_TO_8(
        APPEARANCE_WATCH_SPORTS) };
uint8_t golf_glove_golf_glove_next_frame[] = { 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00 };
uint8_t golf_glove_golf_glove_next_frame_client_configuration[] = { BIT16_TO_8(
        GATT_CLIENT_CONFIG_NONE) };
uint8_t golf_glove_golf_glove_realtime_enabled[] = { 0x00 };
uint8_t golf_glove_golf_glove_realtime_enabled_client_configuration[] = {
        BIT16_TO_8(GATT_CLIENT_CONFIG_NONE) };
uint8_t golf_glove_golf_glove_data_available[] = { 0x00 };
uint8_t golf_glove_golf_glove_data_available_client_configuration[] = {
        BIT16_TO_8(GATT_CLIENT_CONFIG_NONE) };
...
const uint16_t golf_glove_gatt_db_ext_attr_tbl_size =
        (sizeof(golf_glove_gatt_db_ext_attr_tbl) / sizeof(gatt_db_lookup_table));
...
wiced_bt_dev_status_t golf_glove_management_callback(
    ...
    case BTM_ENABLED_EVT:
        ...
        WICED_BT_TRACE("Bluetooth Enabled (%s)\r\n",
                ((WICED_BT_SUCCESS == p_event_data->enabled.status) ?
                        "success" : "failure"));
...
/* Get a Value */
wiced_bt_gatt_status_t golf_glove_get_value(uint16_t attr_handle,
        ...
        if (golf_glove_gatt_db_ext_attr_tbl[i].handle == attr_handle) {
            ...
            if (golf_glove_gatt_db_ext_attr_tbl[i].cur_len <= max_len) {
                ...
                case HDLC_GOLF_GLOVE_REALTIME_ENABLED_VALUE:
                    break;
                case HDLD_GOLF_GLOVE_REALTIME_ENABLED_CLIENT_CONFIGURATION:
                    break;
                ...
                *p_len = golf_glove_gatt_db_ext_attr_tbl[i].cur_len;
                memcpy(p_val, golf_glove_gatt_db_ext_attr_tbl[i].p_data,
                        golf_glove_gatt_db_ext_attr_tbl[i].cur_len);
...
/* Set a Value */
wiced_bt_gatt_status_t golf_glove_set_value(uint16_t attr_handle,
...
wiced_bt_gatt_status_t golf_glove_write_handler(
        wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id) {
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Write Request */
    status = golf_glove_set_value(p_write_req->handle, conn_id,
...
wiced_bt_gatt_status_t golf_glove_read_handler(wiced_bt_gatt_read_t *p_read_req,
        uint16_t conn_id) {
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Read Request */
    status = golf_glove_get_value(p_read_req->handle, conn_id,
            p_read_req->p_val, *p_read_req->p_val_len, p_read_req->p_val_len);

    return status;
}

/* GATT Connection Status Callback */
wiced_bt_gatt_status_t golf_glove_connect_callback(
    ...
    if ( NULL != p_conn_status) {
        if (p_conn_status->connected) {
            ...
            WICED_BT_TRACE(
                    "Disconnected : BDA '%B', Connection ID '%d', Reason '%d'\r\n",
                    p_conn_status->bd_addr, p_conn_status->conn_id,
...
wiced_bt_gatt_status_t golf_glove_server_callback(uint16_t conn_id,
...
/* GATT Event Handler */
wiced_bt_gatt_status_t golf_glove_event_handler(wiced_bt_gatt_evt_t event,
    ...
    case GATT_ATTRIBUTE_REQUEST_EVT:
        ...
        status = golf_glove_server_callback(p_attr_req->conn_id,
...
uint32_t hci_control_process_rx_cmd(uint8_t* p_data, uint32_t len) {
    ...
    if ((NULL == p_data) || (len < 4)) {
        ...
        payload_length = LITTLE_ENDIAN_BYTE_ARRAY_TO_UINT16(
        ...
        default:
            ...
            wiced_transport_send_data(HCI_CONTROL_EVENT_COMMAND_STATUS,
